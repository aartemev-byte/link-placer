...
function drupalOpenEdit(site){
  ...
  if(dest[0]!=='/')dest='/'+dest;
  var fullEdit=baseUrl(site)+dest;
...
async function doCheck(sid){var site=gs(sid);if(!site)return;D.ckid=sid;
  ...
  var ipEl=document.getElementById('inject-path');
  if(ipEl){var ip=ipEl.value.trim();if(ip){if(ip[0]!=='/')ip='/'+ip;for(var i=0;i<D.sites.length;i++){if(D.sites[i].id===sid)D.sites[i].injectPath=ip}site=gs(sid)}}
  ...
  if(!connected){
    var eps=[
      {url:baseUrl(site)+'/wp-json/lp-api/v2/verify',m:'plugin'},
      {url:baseUrl(site)+'/lp-connector.php?action=verify',m:'file'},
      {url:baseUrl(site)+'/lp-modx.php?action=verify',m:'file-modx'},
      {url:baseUrl(site)+'/?lp_action=verify',m:'modx-plugin'}
    ];
    // Add inject endpoint if path is set
    if(site.injectPath){eps.push({url:baseUrl(site)+site.injectPath+'?lp_action=verify',m:'inject'})}
...
async function doRemoveLink(sid,linkId,linkIdx){var site=gs(sid);if(!site)return;
  ...
  else if(m==='file')url=baseUrl(site)+'/lp-connector.php?action=remove';
  else url=baseUrl(site)+'/wp-json/lp-api/v2/remove';
...
// ========== MODX INJECTABLE CODE ==========
function mkModxInject(s){return '// --- LinkForge v3 Start ---\nif (isset($_GET[\'lp_action\']) && $_GET[\'lp_action\'] !== \'\') {\n    $__lf_secret = \''+s+'\';\n    $__lf_file = dirname(__FILE__) . \'/lp_links_data.json\';\n    $__lf_action = $_GET[\'lp_action\'];\n\n    function __lf_read($f) {\n        if (!file_exists($f)) return array();\n        $d = json_decode(file_get_contents($f), true);\n        return is_array($d) ? $d : array();\n    }\n    function __lf_write($f, $data) {\n        file_put_contents($f, json_encode($data, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));\n    }\n\n    header(\'Content-Type: application/json; charset=utf-8\');\n    header(\'Access-Control-Allow-Origin: *\');\n    header(\'Access-Control-Allow-Headers: Content-Type, X-LP-Secret\');\n    header(\'Access-Control-Allow-Methods: GET, POST, OPTIONS\');\n\n    if ($_SERVER[\'REQUEST_METHOD\'] === \'OPTIONS\') {\n        http_response_code(200);\n        exit;\n    }\n\n    $__lf_sec = isset($_SERVER[\'HTTP_X_LP_SECRET\']) ? $_SERVER[\'HTTP_X_LP_SECRET\'] : \'\';\n    $__lf_input = json_decode(file_get_contents(\'php://input\'), true);\n    if (!is_array($__lf_input)) $__lf_input = array();\n\n    if ($__lf_action === \'verify\') {\n        if ($__lf_sec !== $__lf_secret) { http_response_code(403); echo json_encode(array(\'error\'=>\'forbidden\')); exit; }\n        echo json_encode(array(\'verified\' => isset($__lf_input[\'token\']) ? $__lf_input[\'token\'] : \'\', \'method\' => \'inject\', \'cms\' => \'modx\'));\n        exit;\n    }\n    if ($__lf_sec !== $__lf_secret) { http_response_code(403); echo json_encode(array(\'error\'=>\'forbidden\')); exit; }\n    if ($__lf_action === \'place\') {\n        $html = isset($__lf_input[\'html\']) ? $__lf_input[\'html\'] : \'\';\n        $pl = isset($__lf_input[\'placement\']) ? $__lf_input[\'placement\'] : \'homepage\';\n        if (!$html) { http_response_code(400); echo json_encode(array(\'error\'=>\'need html\')); exit; }\n        $html = strip_tags($html, \'<a><b><strong><i><em><span><div><p><br><img><ul><ol><li>\');\n        $ls = __lf_read($__lf_file);\n        $id = \'lp_\' . uniqid();\n        $ls[$id] = array(\'html\' => $html, \'pl\' => $pl, \'t\' => date(\'Y-m-d H:i:s\'));\n        __lf_write($__lf_file, $ls);\n        echo json_encode(array(\'status\' => \'placed\', \'id\' => $id));\n        exit;\n    }\n    if ($__lf_action === \'remove\') {\n        $id = isset($__lf_input[\'id\']) ? $__lf_input[\'id\'] : \'\';\n        $ls = __lf_read($__lf_file);\n        unset($ls[$id]);\n        __lf_write($__lf_file, $ls);\n        echo json_encode(array(\'status\' => \'removed\'));\n        exit;\n    }\n}\n// --- LinkForge v3 End ---'}

// ========== MODX ALL-IN-ONE PLUGIN (Revolution) ==========
function mkModxAllInOne(s){return '<?php\n/**\n * LinkForge v3 — All-in-one Plugin for MODX Revolution\n * Events: OnWebPageInit, OnWebPagePrerender\n *\n * Handles API requests (verify, place, remove) and link output\n * without any additional files. Just create this plugin in MODX admin.\n *\n * System Events to enable:\n *   - OnWebPageInit\n *   - OnWebPagePrerender\n */\n\n$secret = \''+s+'\';\n$sf = MODX_BASE_PATH . \'lp_links_data.json\';\n\n// === Helper functions ===\nif (!function_exists(\'lp_read\')) {\n    function lp_read($f) {\n        if (!file_exists($f)) return array();\n        $d = json_decode(file_get_contents($f), true);\n        return is_array($d) ? $d : array();\n    }\n    function lp_write($f, $data) {\n        file_put_contents($f, json_encode($data, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));\n    }\n    function lp_sanitize($html) {\n        return strip_tags($html, \'<a><b><strong><i><em><span><div><p><br><img><ul><ol><li>\');\n    }\n}\n\n$eventName = $modx->event->name;\n\n// =============================================\n// OnWebPageInit — intercept API requests\n// =============================================\nif ($eventName === \'OnWebPageInit\') {\n    $action = isset($_GET[\'lp_action\']) ? $_GET[\'lp_action\'] : \'\';\n    if (!$action) return; // not our request, skip\n\n    // CORS headers\n    header(\'Content-Type: application/json; charset=utf-8\');\n    header(\'Access-Control-Allow-Origin: *\');\n    header(\'Access-Control-Allow-Headers: Content-Type, X-LP-Secret\');\n    header(\'Access-Control-Allow-Methods: GET, POST, OPTIONS\');\n\n    if ($_SERVER[\'REQUEST_METHOD\'] === \'OPTIONS\') {\n        http_response_code(200);\n        @session_write_close();\n        exit;\n    }\n\n    $sec = isset($_SERVER[\'HTTP_X_LP_SECRET\']) ? $_SERVER[\'HTTP_X_LP_SECRET\'] : \'\';\n    $input = json_decode(file_get_contents(\'php://input\'), true);\n    if (!is_array($input)) $input = array();\n\n    // --- verify ---\n    if ($action === \'verify\') {\n        if ($sec !== $secret) {\n            http_response_code(403);\n            echo json_encode(array(\'error\' => \'forbidden\'));\n            @session_write_close();\n            exit;\n        }\n        $token = isset($input[\'token\']) ? $input[\'token\'] : \'\';\n        echo json_encode(array(\n            \'verified\' => $token,\n            \'method\'   => \'modx-plugin\',\n            \'cms\'      => \'modx-revolution\'\n        ));\n        @session_write_close();\n        exit;\n    }\n\n    // Auth check for place/remove\n    if ($sec !== $secret) {\n        http_response_code(403);\n        echo json_encode(array(\'error\' => \'forbidden\'));\n        @session_write_close();\n        exit;\n    }\n\n    // --- place ---\n    if ($action === \'place\') {\n        $html = isset($input[\'html\']) ? $input[\'html\'] : \'\';\n        $pl   = isset($input[\'placement\']) ? $input[\'placement\'] : \'homepage\';\n        if (!$html) {\n            http_response_code(400);\n            echo json_encode(array(\'error\' => \'need html\'));\n            @session_write_close();\n            exit;\n        }\n        $html = lp_sanitize($html);\n        $ls = lp_read($sf);\n        $id = \'lp_\' . uniqid();\n        $ls[$id] = array(\'html\' => $html, \'pl\' => $pl, \'t\' => date(\'Y-m-d H:i:s\'));\n        lp_write($sf, $ls);\n        echo json_encode(array(\'status\' => \'placed\', \'id\' => $id));\n        @session_write_close();\n        exit;\n    }\n\n    // --- remove ---\n    if ($action === \'remove\') {\n        $id = isset($input[\'id\']) ? $input[\'id\'] : \'\';\n        $ls = lp_read($sf);\n        unset($ls[$id]);\n        lp_write($sf, $ls);\n        echo json_encode(array(\'status\' => \'removed\'));\n        @session_write_close();\n        exit;\n    }\n\n    // --- links (list) ---\n    if ($action === \'links\') {\n        echo json_encode(lp_read($sf));\n        @session_write_close();\n        exit;\n    }\n}\n\n// =============================================\n// OnWebPagePrerender — output links in HTML\n// =============================================\nif ($eventName === \'OnWebPagePrerender\') {\n    if (!file_exists($sf)) return;\n    $ls = json_decode(file_get_contents($sf), true);\n    if (!is_array($ls) || empty($ls)) return;\n\n    $isHome = ($modx->resource && $modx->resource->get(\'id\') == $modx->getOption(\'site_start\'));\n    $inject = \'\';\n    foreach ($ls as $l) {\n        $pl = isset($l[\'pl\']) ? $l[\'pl\'] : \'homepage\';\n        if ($pl === \'homepage\' && !$isHome) continue;\n        $inject .= isset($l[\'html\']) ? $l[\'html\'] : \'\';\n    }\n    if ($inject) {\n        $o = &$modx->resource->_output;\n        $o = str_replace(\'</body>\', $inject . \'</body>\', $o);\n    }\n}'}

// ========== MODX ALL-IN-ONE PLUGIN (Evolution) ==========
function mkModxEvoAllInOne(s){return '<?php\n/**\n * LinkForge v3 — All-in-one Plugin for MODX Evolution\n * Events: OnWebPageInit, OnWebPagePrerender\n *\n * Handles API requests and link output in one plugin.\n * No additional files needed.\n *\n * System Events to enable:\n *   - OnWebPageInit\n *   - OnWebPagePrerender\n */\n\n$secret = \''+s+'\';\n$sf = MODX_BASE_PATH . \'lp_links_data.json\';\n\nif (!function_exists(\'lp_read\')) {\n    function lp_read($f) {\n        if (!file_exists($f)) return array();\n        $d = json_decode(file_get_contents($f), true);\n        return is_array($d) ? $d : array();\n    }\n    function lp_write($f, $data) {\n        file_put_contents($f, json_encode($data, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));\n    }\n    function lp_sanitize($html) {\n        return strip_tags($html, \'<a><b><strong><i><em><span><div><p><br><img><ul><ol><li>\');\n    }\n}\n\n$e = $modx->event->name;\n\n// =============================================\n// OnWebPageInit — intercept API requests\n// =============================================\nif ($e === \'OnWebPageInit\') {\n    $action = isset($_GET[\'lp_action\']) ? $_GET[\'lp_action\'] : \'\';\n    if (!$action) return;\n\n    header(\'Content-Type: application/json; charset=utf-8\');\n    header(\'Access-Control-Allow-Origin: *\');\n    header(\'Access-Control-Allow-Headers: Content-Type, X-LP-Secret\');\n    header(\'Access-Control-Allow-Methods: GET, POST, OPTIONS\');\n\n    if ($_SERVER[\'REQUEST_METHOD\'] === \'OPTIONS\') {\n        http_response_code(200);\n        @session_write_close();\n        exit;\n    }\n\n    $sec = isset($_SERVER[\'HTTP_X_LP_SECRET\']) ? $_SERVER[\'HTTP_X_LP_SECRET\'] : \'\';\n    $input = json_decode(file_get_contents(\'php://input\'), true);\n    if (!is_array($input)) $input = array();\n\n    if ($action === \'verify\') {\n        if ($sec !== $secret) {\n            http_response_code(403);\n            echo json_encode(array(\'error\' => \'forbidden\'));\n            @session_write_close();\n            exit;\n        }\n        $token = isset($input[\'token\']) ? $input[\'token\'] : \'\';\n        echo json_encode(array(\n            \'verified\' => $token,\n            \'method\'   => \'modx-plugin-evo\',\n            \'cms\'      => \'modx-evo\'\n        ));\n        @session_write_close();\n        exit;\n    }\n\n    if ($sec !== $secret) {\n        http_response_code(403);\n        echo json_encode(array(\'error\' => \'forbidden\'));\n        @session_write_close();\n        exit;\n    }\n\n    if ($action === \'place\') {\n        $html = isset($input[\'html\']) ? $input[\'html\'] : \'\';\n        $pl   = isset($input[\'placement\']) ? $input[\'placement\'] : \'homepage\';\n        if (!$html) {\n            http_response_code(400);\n            echo json_encode(array(\'error\' => \'need html\'));\n            @session_write_close();\n            exit;\n        }\n        $html = lp_sanitize($html);\n        $ls = lp_read($sf);\n        $id = \'lp_\' . uniqid();\n        $ls[$id] = array(\'html\' => $html, \'pl\' => $pl, \'t\' => date(\'Y-m-d H:i:s\'));\n        lp_write($sf, $ls);\n        echo json_encode(array(\'status\' => \'placed\', \'id\' => $id));\n        @session_write_close();\n        exit;\n    }\n\n    if ($action === \'remove\') {\n        $id = isset($input[\'id\']) ? $input[\'id\'] : \'\';\n        $ls = lp_read($sf);\n        unset($ls[$id]);\n        lp_write($sf, $ls);\n        echo json_encode(array(\'status\' => \'removed\'));\n        @session_write_close();\n        exit;\n    }\n\n    if ($action === \'links\') {\n        echo json_encode(lp_read($sf));\n        @session_write_close();\n        exit;\n    }\n}\n\n// =============================================\n// OnWebPagePrerender — output links\n// =============================================\nif ($e === \'OnWebPagePrerender\') {\n    if (!file_exists($sf)) return;\n    $ls = json_decode(file_get_contents($sf), true);\n    if (!is_array($ls) || empty($ls)) return;\n\n    $isHome = ($modx->documentIdentifier == $modx->config[\'site_start\']);\n    $inject = \'\';\n    foreach ($ls as $l) {\n        $pl = isset($l[\'pl\']) ? $l[\'pl\'] : \'homepage\';\n        if ($pl === \'homepage\' && !$isHome) continue;\n        $inject .= isset($l[\'html\']) ? $l[\'html\'] : \'\';\n    }\n    if ($inject) {\n        $modx->documentOutput = str_replace(\'</body>\', $inject . \'</body>\', $modx->documentOutput);\n    }\n}'}

// ========== LEGACY MODX code generators (kept for backward compat) ==========
function mkModxPlugin(s){return '<?php\n// LinkForge Plugin MODX Revo — OnWebPagePrerender\n$sf = MODX_BASE_PATH . \'lp_links_data.json\';\nif (!file_exists($sf)) return;\n$ls = json_decode(file_get_contents($sf), true);\nif (!is_array($ls) || empty($ls)) return;\n$home = ($modx->resource && $modx->resource->get(\'id\') == $modx->getOption(\'site_start\'));\n$h = \'\';\nforeach ($ls as $l) {\n    $pl = isset($l[\'pl\']) ? $l[\'pl\'] : \'homepage\';\n    if ($pl === \'homepage\' && !$home) continue;\n    $h .= isset($l[\'html\']) ? $l[\'html\'] : \'\';\n}\nif ($h) {\n    $o = &$modx->resource->_output;\n    $o = str_replace(\'</body>\', $h . \'</body>\', $o);\n}'}
function mkModxSnippet(s){return '<?php\n// LinkForge Snippet MODX Revo — [[LinkForge]] or [[LinkForge? &placement=`all`]]\n$pl = $modx->getOption(\'placement\', $scriptProperties, \'auto\');\n$sf = MODX_BASE_PATH . \'lp_links_data.json\';\nif (!file_exists($sf)) return \'\';\n$ls = json_decode(file_get_contents($sf), true);\nif (!is_array($ls) || empty($ls)) return \'\';\n$home = ($modx->resource && $modx->resource->get(\'id\') == $modx->getOption(\'site_start\'));\n$o = \'\';\nforeach ($ls as $l) {\n    $lp = isset($l[\'pl\']) ? $l[\'pl\'] : \'homepage\';\n    if ($pl === \'auto\') {\n        if ($lp === \'homepage\' && !$home) continue;\n    } elseif ($pl !== \'all\' && $lp !== $pl) {\n        continue;\n    }\n    $o .= isset($l[\'html\']) ? $l[\'html\'] : \'\';\n}\nreturn $o;'}
function mkModxEvoPlugin(s){return '<?php\n// LinkForge Plugin MODX Evo — OnWebPagePrerender\n$sf = MODX_BASE_PATH . \'lp_links_data.json\';\nif (!file_exists($sf)) return;\n$ls = json_decode(file_get_contents($sf), true);\nif (!is_array($ls) || empty($ls)) return;\n$home = ($modx->documentIdentifier == $modx->config[\'site_start\']);\n$h = \'\';\nforeach ($ls as $l) {\n    $pl = isset($l[\'pl\']) ? $l[\'pl\'] : \'homepage\';\n    if ($pl === \'homepage\' && !$home) continue;\n    $h .= isset($l[\'html\']) ? $l[\'html\'] : \'\';\n}\nif ($h) {\n    $modx->documentOutput = str_replace(\'</body>\', $h . \'</body>\', $modx->documentOutput);\n}'}
...
function getConnectOptions(cms){
  if(cms==='MODX Revolution')return[
    ...
    {key:'modx-inject',icon:'💉',bg:'linear-gradient(135deg,var(--cyan),#0891b2)',title:'Вставить код в PHP-файл',desc:'В любой .php файл сайта'}
  ];
...
function getConnectContent(key,site){var sec=site.secret;
  var map={
    ...
    'modx-allinone':{title:'Плагин «всё-в-одном» (MODX Revo)',steps:[
      'Откройте админку MODX',
      'Элементы → Плагины → Новый плагин',
      'Название: <code>LinkForge</code>',
      'Вставьте код ниже в поле плагина',
      'Вкладка «Системные события» → включите: <code>OnWebPageInit</code> и <code>OnWebPagePrerender</code>',
      'Сохраните плагин',
      'Нажмите «Проверить связь» ниже'
    ],code:mkModxAllInOne(sec),note:'Никаких файлов загружать не нужно! Плагин сам обрабатывает API-запросы и выводит ссылки.'},

    // --- MODX Evo All-in-One ---
    'evo-allinone':{title:'Плагин «всё-в-одном» (MODX Evo)',steps:[
      'Откройте админку MODX Evolution',
      'Элементы → Управление элементами → Плагины → Новый плагин',
      'Название: <code>LinkForge</code>',
      'Вставьте код ниже',
      'Конфигурация → Системные события → включите: <code>OnWebPageInit</code> и <code>OnWebPagePrerender</code>',
      'Сохраните',
      'Нажмите «Проверить связь» ниже'
    ],code:mkModxEvoAllInOne(sec),note:'Никаких файлов загружать не нужно! Плагин сам обрабатывает API-запросы и выводит ссылки.'},

    // --- Inject code methods ---
    'modx-inject':{title:'Вставить код в любой PHP-файл',steps:[
      'Скопируйте код ниже',
      'Откройте любой <code>.php</code> файл сайта (например <code>index.php</code>)',
      'Вставьте код <strong>сразу после</strong> открывающего <code>&lt;?php</code>',
      'Сохраните файл',
    ...
    'evo-inject':{title:'Вставить код в любой PHP-файл',steps:[
      'Скопируйте код ниже',
      'Откройте любой <code>.php</code> файл сайта (например <code>index.php</code>)',
      'Вставьте код <strong>сразу после</strong> открывающего <code>&lt;?php</code>',
      'Сохраните файл',
...
function rModal(){if(!D.modal)return '';var site=gs(D.modal.sid);if(!site)return '';
  if(D.modal.t==='conn'){
    var v=D.modal.v,opts=getConnectOptions(site.cms),inner='';
    if(!v){
      inner='<p style="color:var(--text2);font-size:14px;margin-bottom:20px">Подключение для <strong style="color:var(--text)">'+esc(site.cms)+'</strong></p><div style="display:flex;flex-direction:column;gap:10px">';
      opts.forEach(function(o){
        inner+='<button class="co'+(o.badge?' co-rec':'')+'" data-cv="'+o.key+'">'
          +'<div class="ci" style="background:'+o.bg+'">'+o.icon+'</div>'
          +'<div style="flex:1"><div style="display:flex;align-items:center;gap:8px"><span style="color:var(--text);font-weight:700;font-size:15px">'+o.title+'</span>'
          +(o.badge?'<span class="badge-rec">'+o.badge+'</span>':'')
          +'</div><div style="color:var(--text2);font-size:12px;margin-top:4px">'+o.desc+'</div></div></button>'
      });
      inner+='</div>';
    }else{
      var cc=getConnectContent(v,site);
      inner='<button class="btn bg bsm" data-cv="back">← Назад</button><h3 style="color:var(--text);margin:16px 0 10px;font-size:16px">'+cc.title+'</h3><ol class="st">';
      cc.steps.forEach(function(s){inner+='<li>'+s+'</li>'});
      inner+='</ol>';
      if(cc.dlBtn)inner+='<button class="btn bp" style="margin-top:16px" id="'+cc.dlBtn.id+'">'+cc.dlBtn.label+'</button>';
...
function bindAll(){document.querySelectorAll('[data-tab]').forEach(function(e){e.onclick=function(){D.tab=e.dataset.tab;render()}});var o=document.getElementById('out');if(o)o.onclick=function(){D.li=false;D.cu=null;render()};var ab=document.getElementById('abtn');if(ab)ab.onclick=function(){var n=document.getElementById('an').value.trim(),u=document.getElementById('au').value.trim();if(!n||!u)return notify('Заполните','error');if(u.indexOf('http')!==0)u='https://'+u;u=u.replace(/\/+$/,'');if(D.sites.some(function(s){return s.url===u}))return notify('Уже есть','error');D.sites.push({id:Date.now().toString(),url:u,name:n,cms:document.getElementById('ac').value,secret:gensec(),connected:false,connectionMethod:null,links:[],addedAt:new Date().toISOString(),lastCheck:null});log('Добавлен',u);D.tab='sites';notify('✅ Добавлен','success');render()};var ea=document.getElementById('exp-all');if(ea)ea.onclick=function(){exportAll()};document.querySelectorAll('[data-do]').forEach(function(e){e.onclick=function(){var id=e.dataset.id,idx=parseInt(e.dataset.idx),act=e.dataset.do;if(act==='conn'){D.modal={t:'conn',sid:id,v:null};render()}if(act==='chk')doCheck(id);if(act==='place'){D.modal={t:'place',sid:id};render()}if(act==='del'){if(confirm('Удалить?')){D.sites=D.sites.filter(function(s){return s.id!==id});sv();notify('Удалён');render()}}if(act==='delu'){D.users.splice(idx,1);sv();notify('Удалён');render()}if(act==='export')exportSite(id);if(act==='rmlink'){var sid=e.dataset.sid,lid=e.dataset.lid,lidx=parseInt(e.dataset.lidx);if(confirm('Удалить ссылку?'))doRemoveLink(sid,lid,lidx)}}});var cx=document.getElementById('cx');if(cx)cx.onclick=function(){D.modal=null;render()};var ov=document.getElementById('ov');if(ov)ov.onclick=function(e){if(e.target===ov){D.modal=null;render()}};document.querySelectorAll('[data-cv]').forEach(function(e){e.onclick=function(){if(D.modal)D.modal.v=e.dataset.cv==='back'?null:e.dataset.cv;render()}});if(D.modal&&D.modal.v){var site=gs(D.modal.sid);if(site){var cc=getConnectContent(D.modal.v,site);var dlb=document.getElementById('dl1');if(dlb&&cc.dlBtn)dlb.onclick=cc.dlBtn.fn;var cpb=document.getElementById('cpcode');if(cpb&&cc.code)cpb.onclick=function(){cpTxt(cc.code)}}}var mchk=document.getElementById('mchk');if(mchk&&D.modal)mchk.onclick=function(){
    ...
    var dgo=document.getElementById('drupal-go-edit');if(dgo&&D.modal){dgo.onclick=function(){var s=gs(D.modal.sid);if(s){var u=s.drupalEditUrl||'';if(!u)return notify('Укажите URL','error');if(u.indexOf('http')!==0)u=baseUrl(s)+(u[0]==='/'?'':'/')+u;window.open(u,'_blank');log('Drupal →',u)}}}
